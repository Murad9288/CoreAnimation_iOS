...
class Animation: NSObject, CAAnimationDelegate {
    ...
    class func bounce(value: Float, delay: TimeInterval) -> Animation {
        let animation = Animation()
    ...
    class func movePosition(position: CGPoint, delay: TimeInterval) -> Animation {
        let animation = Animation()
    ...
    class func resizeFrame(resize: CGSize, delay: TimeInterval) -> Animation {
        let animation = Animation()
    ...
    class func rotation(angle: Float, delay: TimeInterval) -> Animation {
        let animation = Animation()
    ...
    class func rotationY(rotation: Float, delay: TimeInterval) -> Animation {
        let animation = Animation()
    ...
    class func rotationX(rotation: Float, delay: TimeInterval) -> Animation {
        let animation = Animation()
    ...
    class func rotationZ(rotation: Float, delay: TimeInterval) -> Animation {
        let animation = Animation()
    ...
    class func fade(pourcent: Float, delay: TimeInterval) -> Animation {
        let animation = Animation()
    ...
    class func borderRaduis(angle: Float, delay: TimeInterval) -> Animation {
        let animation = Animation()
    ...
    class func moveCircle(frameCircle: CGRect, delay: TimeInterval) -> Animation {
        let animation = Animation()
    ...
    private func createAnimation(pathAnimation: String, delay: TimeInterval) -> CABasicAnimation {
    ...
    func runBounce(layer:CALayer, value:Float, blockCompletion: (() -> ())?) {
        if value <= 3 {
            ...
            layer.frame = CGRectMake(layer.frame.origin.x - CGFloat(value / 2), layer.frame.origin.y - CGFloat(value / 2),
                ...
                UIView.animate(withDuration: self.delay, animations: { () -> Void in
                    layer.frame = CGRectMake(layer.frame.origin.x + CGFloat(value / 2), layer.frame.origin.y + CGFloat(value / 2),
                        ...
                        self.runBounce(layer: layer, value: value / 2, blockCompletion: blockCompletion)
                })
        }
    }
    ...
    private func runRotation(layer:CALayer, rotation: Float, blockCompletion: (() -> ())?) {
        let animation = createAnimation(pathAnimation: "transform.rotation", delay: self.delay)
        
        animation.fromValue = 0
        if let currentLayer: AnyObject = layer.presentation() {
            animation.fromValue = Float(truncating: currentLayer.value(forAnimatedKey: "transform.rotation") as NSNumber)
            
        }
        
        self.blockCompletion = blockCompletion
        animation.toValue = rotation + Float(truncating: animation.fromValue as! NSNumber)
        layer.add(animation, forKey: "rotation")
    }
    
    private func runRotationY(layer: CALayer, y: Float, blockCompletion: (() -> ())?) {
        let animation = createAnimation(pathAnimation: "transform.rotation.y", delay: self.delay)
        ...
        animation.toValue = y + Float(truncating: animation.fromValue as! NSNumber)
        layer.add(animation, forKey: "rotationY")
        layer.transform = rotation
    }

    private func runRotationX(layer: CALayer, x: Float, blockCompletion: (() -> ())?) {
        let animation = createAnimation(pathAnimation: "transform.rotation.x", delay: self.delay)
        var rotation = CATransform3DIdentity
        ...
        self.blockCompletion = blockCompletion
        rotation.m34 = 1.0 / 500.0
        animation.toValue = x + Float(truncating: animation.fromValue as! NSNumber)
        layer.add(animation, forKey: "rotationX")
        layer.transform = rotation
    }
    
    private func runRotationZ(layer: CALayer, z: Float, blockCompletion: (() -> ())?) {
        let animation = createAnimation(pathAnimation: "transform.rotation.z", delay: self.delay)
        var rotation = CATransform3DIdentity
        ...
        self.blockCompletion = blockCompletion
        rotation.m34 = 1.0 / 500.0
        animation.toValue = z + Float(truncating: animation.fromValue as! NSNumber)
        layer.add(animation, forKey: "rotationZ")
        layer.transform = rotation
    }
    
    private func runFade(layer: CALayer, pourcent: Float, blockCompletion: (() -> ())?) {
        let animation = createAnimation(pathAnimation: "opacity", delay: self.delay)
    ...
    private func runBorderRaduis(layer: CALayer, angle: Float, blockCompletion: (() -> ())?) {
        let animation = createAnimation(pathAnimation: "cornerRadius", delay: self.delay)
    ...
    private func execAnimationSequence(layer:CALayer) {
        if self.countAnimation >= self.animationsList?.count ?? self.countAnimation {
        ...
        currentAnimation?.runAnimation(layer: layer, blockCompletion: { () -> () in
            self.countAnimation += 1
            self.execAnimationSequence(layer: layer)
        })
    }
    ...
    private func runRepeatAnimation(layer: CALayer, count: Int, animation: Animation, blockCompletion: (() -> ())?) {
        ...
        animation.runAnimation(layer: layer, blockCompletion: { () -> () in
            self.runRepeatAnimation(layer: layer, count: count - 1, animation: animation, blockCompletion: blockCompletion)
        })
    }
    
    func runAnimation(layer: CALayer, blockCompletion: (() -> ())?) {
        switch self.type {
        case .Bounce(let value):
            self.runBounce(layer: layer, value: value, blockCompletion: blockCompletion)
        case .Position(let value):
            self.runMovePosition(layer: layer, position: value, blockCompletion: blockCompletion)
        case .Resize(let value):
            self.runResize(layer: layer, resize: value, blockCompletion: blockCompletion)
        case .Rotation(let value):
            self.runRotation(layer: layer, rotation: value, blockCompletion: blockCompletion)
        case .RotationY(let value):
            self.runRotationY(layer: layer, y: value, blockCompletion: blockCompletion)
        case .RotationX(let value):
            self.runRotationX(layer: layer, x: value, blockCompletion: blockCompletion)
        case .RotationZ(let value):
            self.runRotationZ(layer: layer, z: value, blockCompletion: blockCompletion)
        case .Fade(let value):
            self.runFade(layer: layer, pourcent: value, blockCompletion: blockCompletion)
        case .BorderRaduis(let value):
            self.runBorderRaduis(layer: layer, angle: value, blockCompletion: blockCompletion)
        case .Sequence(let animations):
            self.runSequenceAnimation(layer: layer, animations: animations, blockCompletion: blockCompletion)
        case .Repeat((let count, let animation)):
            self.runRepeatAnimation(layer: layer, count: count, animation: animation, blockCompletion: blockCompletion)
        case .MoveCircle(let value):
            self.runMoveCircle(layer: layer, frameCircle: value, blockCompletion: blockCompletion)
        case .None:
            Void()
        }
    }
...
extension CALayer {
    ...
    func runAnimation(animation: Animation) {
        animation.runAnimation(layer: self, blockCompletion: nil)
    }

    func runAnimation(animation: Animation, blockCompletion :@escaping (() -> ())) {
        animation.runAnimation(layer: self, blockCompletion: blockCompletion)
    }
}
